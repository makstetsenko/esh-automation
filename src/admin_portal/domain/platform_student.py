...
class PlatformStudent(BaseModel):
...
def write_to_csv(students: list[PlatformStudent], path: pathlib.Path) -> None:
    fieldnames = [field.serialization_alias or name for name, field in PlatformStudent.model_fields.items()]

    with open(path.as_posix(), "w", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()

        writer.writerows(student.model_dump(by_alias=True) for student in students)
